...
i = 0
response = requests.get('https://gist.githubusercontent.com/jpriebe/d62a45e29f24e843c974/raw/b1d3066d245e742018bce56e41788ac7afa60e29/us_state_capitals.json')
...
print('<html>')
print('<head>')
print('<title>Weather Map by Ryan Power</title>')
print('</head>')
print('<body>')
print('''
<script>
...
for state in states:
    ...
    if (i<51):
        ...
        long = states[state]['long']
        url = weather + lat + "," + long
        res = requests.get(url)
        data = json.loads(res.text)
        weatherurl = data['properties']['forecast']
        res2 = requests.get(weatherurl)
        data2 = json.loads(res2.text)
        try:
            temp = data2['properties']['periods'][0]['temperature']
            if (temp < 10):
                print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#1C7BF5');")
            elif (temp < 30):
                print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#1CEEF5');")
            elif (temp < 50):
                print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#3AF51C');")
            elif (temp < 80):
                print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#F58C1C');")
            else:
                print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#F51C1C');")
        except:
            print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#808080');")
    else:
        print(f"document.querySelector('.{state.lower()}').setAttribute('fill', '#808080');")
print('''
};
